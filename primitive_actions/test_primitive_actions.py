...
class TestPrimitiveActions(Node):
    ...
    def _send_pick(self, x, y, z, yaw):
        ...
        goal.z   = float(z)
        goal.yaw = float(yaw)
        goal.approach_height = 0.0  # use server default

        self.get_logger().info(
            f'Sending Pick  x={x}  y={y}  z={z}  yaw={yaw:.4f}')
    ...
    def _send_place(self, x, y, z, yaw):
        self.get_logger().info(
        ...
        self.get_logger().info(
            f'Sending Place x={x}  y={y}  z={z}  yaw={yaw:.4f}')
    ...
    def run(self):
        ok = self._send_pick(x=0.4, y=-0.2, z=0.05, yaw=0.0)
        if not ok:
            self.get_logger().error('Aborting: pick failed.')
            return

        ok = self._send_place(x=0.4, y=0.2, z=0.05, yaw=math.pi / 2)
        if not ok:
